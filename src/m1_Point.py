...
class Point(object):
    ...
    def move_to(self, x, y):
        self.distance_traveled = self.distance_traveled + ((((self.x - x) ** 2) + ((self.y - y) ** 2)) ** 0.5)
        self.x = x
        self.y = y
    ...
    def move_by(self, dx, dy):
        ...
        self.y = self.y + dy
        self.moves = self.moves + 1
        self.distance_traveled = self.distance_traveled + (((dx ** 2) + (dy ** 2)) ** 0.5)
    def get_number_of_moves_made(self):
        return self.moves
    def get_distance_from(self, point2):
        self.distance = ((((self.x - point2.x) ** 2) + ((self.y - point2.y) ** 2)) ** 0.5)
        return self.distance
    def get_distance_from_start(self):
        self.distance_start = ((((self.x - self.start_x) ** 2) + ((self.y - self.start_y) ** 2)) ** 0.5)
